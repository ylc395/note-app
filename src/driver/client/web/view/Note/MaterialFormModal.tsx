...
export default function MaterialFormModal() {
  const { newMaterialForm } = container.resolve(NoteService);
  const duplicatedNotes = createMemo(() => newMaterialForm.form?.duplicatedNotesQuery.result.data || []);

  async function handleFileSelected(file?: File) {
    assert(newMaterialForm.form);
    newMaterialForm.form.setFile(
      file && {
  ...
  return (
    <Modal
      title="创建素材"
      open={Boolean(newMaterialForm.form)}
      closeOnInteractOutside={false}
      confirmText="创 建"
      canConfirm={!newMaterialForm.form?.isValid}
      onConfirm={() => newMaterialForm.form?.submit()}
      onCancel={() => newMaterialForm.form?.destroy()}
      onClose={() => newMaterialForm.form?.destroy()}
    >
      <form class="space-y-4 form">
        <Field.Root>
          <Field.Label>位于</Field.Label>
          <span class="text-sm" title={newMaterialForm.form?.path?.map(({ title }) => title).join('/')}>
            {last(newMaterialForm.form?.path)?.title || '根目录'}
          </span>
        </Field.Root>
        <Field.Root>
          ...
          <Field.Input
            class="input"
            value={newMaterialForm.form?.get('title') ?? ''}
            onInput={(e) => newMaterialForm.form?.set('title', e.target.value)}
          />
        </Field.Root>
        <Field.Root>
        ...
        <Field.Root>
          ...
          <div>
            ...
            >
              <Show
                when={newMaterialForm.form?.hasFile}
                fallback={
